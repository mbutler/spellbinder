...
let shield = function(caster, target) {
	let description = caster.name + " casts Shield";
	let protections = ["monster", "missle", "stab"];

	//set current spell
	caster.currentSpell = 'Shield';
...
let stab = function(caster, target) {
	let description = caster.name + " stabs " + target.name + " and does 1 point of damage";

	//set current
	caster.currentSpell = 'Stab';
...
let missile = function(caster, target) {
	description = caster.name + " casts Missile at " + target.name;

	//set current
	caster.currentSpell = 'Missile';

	if (!_.includes(target.protection, "missle", "spell", "magic")) {
		target.hp =- 1;
	}

	if (_.includes(target.protection, "mirror")) {
		caster.hp -= 1;
	}
}
